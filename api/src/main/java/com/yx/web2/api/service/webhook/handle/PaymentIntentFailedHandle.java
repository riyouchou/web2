...
@Service(WebHookEventType.PAYMENT_INTENT_FAILED)
@RequiredArgsConstructor
public class PaymentIntentFailedHandle implements IWebHookHandler {
    ...
    public ResponseEntity handle(StripeObject stripeObject) {
        ...
        KvLogger logger = KvLogger.instance(this)
                ...
                .p(LogFieldConstants.ACTION, WebHookEventType.PAYMENT_INTENT_FAILED)
                .p("OrderInfoStr", orderInfoStr);
        if (StringUtil.isBlank(orderInfoStr)) {
            logger.p(LogFieldConstants.ERR_MSG, "not found metadata").e();
        ...
        for (StripePaymentMetaData.OrderIdInfo orderIdInfo : metaData.getMetaData()) {
            ...
            if (orderPaymentEntities == null || orderPaymentEntities.isEmpty()) {
                kvLogger.p(LogFieldConstants.ERR_MSG, "not found paymentOrder").e();
                return ResponseEntity.notFound().build();
            }
            if (orderPaymentEntities.stream().anyMatch(item -> item.getPaymentStatus().intValue() == InstalmentPaymentStatus.FAILED.getValue())) {
                kvLogger.p(LogFieldConstants.ERR_MSG, "paymentOrder is already failed").i();
                return ResponseEntity.ok().build();
            }
            // update paymentOrder
